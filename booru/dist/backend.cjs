...
var ReaddirpStream = class extends import_node_stream.Readable {
  ...
  constructor(options = {}) {
    ...
    this._relStart = this._root.endsWith(import_node_path.sep) ? this._root.length : this._root.length + 1;
  ...
  _formatEntry(dirent, path2) {
    ...
    const fullPath = (0, import_node_path.join)(path2, basename3);
  ...
  async _getSymlinkEntryType(entry) {
    ...
    try {
      ...
      if (entryRealPathStats.isDirectory()) {
        ...
        if (full.startsWith(entryRealPath) && full[len] === import_node_path.sep) {
...
// booru/src/media-runtime.mjs
var MULTIMEDIA_RUNTIME_UNAVAILABLE = "MULTIMEDIA_RUNTIME_UNAVAILABLE";
function createBooruMediaRuntimeAccess(ctx) {
  ...
  return {
    async requireRuntime() {
      ...
      if (!runtimePromise) {
        runtimePromise = Promise.resolve().then(() => ctx.capabilities.media.requireRuntime()).catch((error) => {
...
function isSystemicMediaRuntimeError(error) {
  return error?.code === MULTIMEDIA_RUNTIME_UNAVAILABLE || error?.code === "PLUGIN_CAPABILITY_UNAVAILABLE" || /runtime multimedia de Nexus no esta disponible/i.test(String(error?.message || ""));
}
...
// booru/src/domain/duplicate-ingest.js
...
function stopThumbnailQueueForSystemicFailure(state, errorMessage) {
...
async function processThumbnailQueueEntry(state, resourceId) {
  ...
  try {
    ...
    const systemicFailure = isSystemicMediaRuntimeError(error) || /No se encontro (Python|worker)|Configura pythonExecutable/i.test(errorMessage);
    if (systemicFailure) {
...
function setEntityVisualSync(db, payload) {
  ...
  const nextVisualSettings = hasLayout ? normalizeBooruEntityVisualSettings({
...
function saveEntityProfileSync(db, payload) {
  ...
  const displayName = hasDisplayName ? normalizeBooruOptionalText(payload?.displayName) : currentDisplayName;
  if (!displayName) {
  ...
  const requestedAliases = Object.prototype.hasOwnProperty.call(payload || {}, "aliasNames") ? normalizeUniqueTextList(payload?.aliasNames) : listEntityAliasesSync(db, kind, entityId);
  const aliases = normalizeEntityAliasesForPrimaryName(kind, displayName, [
    ...requestedAliases,
    ...(kind === "author" || kind === "artist") && normalizeBooruComparableText(displayName) !== normalizeBooruComparableText(currentDisplayName) ? [currentDisplayName] : []
  ]);
  ...
  withTransaction(db, () => {
    if (hasDisplayName && displayName !== currentDisplayName) {
      db.prepare(`UPDATE ${getEntityTable(kind)} SET display_name = ? WHERE id = ?`).run(displayName, entityId);
    }
...
async function pasteClipboardImageToEntitySync(ctx, db, payload, tempFilePath) {
  const kind = normalizeBooruText(payload?.kind);
  const entityId = normalizeBooruText(payload?.entityId);
  if (!ENTITY_TABLES[kind]) {
    throw new Error("El tipo de entidad solicitado no existe en Booru.");
...
async function pasteClipboardMediaSync(ctx, db, payload, tempFilePath) {
...
var booruPlugin = {
  ...
  async activate(ctx) {
    ...
    ctx.ipc.handle("list-resources", async (_event, payload) => {
      const startedAt = performance.now();
      try {
    ...
    ctx.ipc.handle("get-resources-by-ids", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const resourceIds = uniqueBooruIds(payload?.resourceIds);
    ...
    ctx.ipc.handle("prime-visible-thumbnails", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const resourceIds = uniqueBooruIds(payload?.resourceIds);
    ...
    ctx.ipc.handle("list-entities", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const kind = normalizeBooruText(payload?.kind);
    ...
    ctx.ipc.handle("get-entity-profile", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const kind = normalizeBooruText(payload?.kind);
    ...
    ctx.ipc.handle("list-entity-relations", async (_event, payload) => {
      const startedAt = performance.now();
      try {
    ...
    ctx.ipc.handle("list-tags", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
        const query = normalizeBooruOptionalText(payload?.query);
        return createSuccess({
    ...
    ctx.ipc.handle("list-search-suggestions", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("save-social-platform", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("delete-social-platform", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("import-social-platform-icon", async (event, payload) => {
    ...
    ctx.ipc.handle("import-social-platform-icon-file", async (event, payload) => {
    ...
    ctx.ipc.handle("list-recommendations", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("ensure-entity", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
        const kind = normalizeBooruText(payload?.kind);
        const name = normalizeBooruText(payload?.name);
    ...
    ctx.ipc.handle("set-character-universe", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("ensure-character-in-universe", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("set-entity-visual", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("set-entity-visual-layout", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("save-entity-profile", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("disassociate-resources-from-entity", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("exclude-resource-tag", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
    ...
    ctx.ipc.handle("exclude-resource-universe", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("set-fast-classification", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("clear-fast-classification", async (_event, payload) => {
      const state2 = runtimeState;
      const scopeId = normalizeBooruOptionalText(payload?.scopeId);
    ...
    ctx.ipc.handle("ensure-tag", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();
        const name = normalizeBooruText(payload?.name);
    ...
    ctx.ipc.handle("save-resource-metadata", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("save-basic-classification", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("quick-assign-entity", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("paste-clipboard-image-to-entity", async (event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const tempFilePath = assertClipboardTempFilePath(
    ...
    ctx.ipc.handle("discard-clipboard-media", async (event, payload) => {
      try {
        const tempFilePath = assertClipboardTempFilePath(
          ctx.capabilities.external.consume(event, String(payload?.grantId || ""))
        );
    ...
    ctx.ipc.handle("paste-clipboard-media", async (event, payload) => {
      try {
        const db = assertRuntimeDb();
        const tempFilePath = assertClipboardTempFilePath(
          ctx.capabilities.external.consume(event, String(payload?.grantId || ""))
    ...
    ctx.ipc.handle("trash-resources", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("restore-resources", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("purge-resources", async (_event, payload) => {
      const startedAt = performance.now();
      try {
        const db = assertRuntimeDb();
        const mutationContext = createResourceMutationContextSync(db, payload);
    ...
    ctx.ipc.handle("open-in-brave", async (_event, payload) => {
      try {
        const db = assertRuntimeDb();

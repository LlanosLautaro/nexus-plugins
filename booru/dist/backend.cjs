...
function buildResourceListSqlParts(db, section, query) {
  ...
  if (section === "duplicates") {
    ...
    whereClauses.push(`r.classification_state != 'duplicate-review'`);
    whereClauses.push(`r.trashed_at IS NULL`);
    if (section === "pending" && pendingMode === "essential") {
  ...
  const orderBySql = section === "pending" ? pendingMode === "tags" ? `ORDER BY ${pendingSql.manualTagCount} ASC, r.imported_at DESC, r.id ASC` : `ORDER BY ${pendingSql.pendingScore} DESC, r.imported_at DESC, r.id ASC` : section === "trash" ? `ORDER BY r.trashed_at DESC, r.imported_at DESC, r.id ASC` : `ORDER BY r.imported_at DESC, r.id ASC`;
  return {
    whereSql,
    parameters,
    orderBySql

...
function assignmentMatchesItemByPath(assignment, itemRelativePath) {
  const assignmentRoot = normalizeRelativePath2(String(assignment?.rootPath || ""));
...
async function isBooksAssignedItemWithAssignments(ctx, item, assignments) {
  const itemRelativePath = normalizeRelativePath2(
...
// Books/src/cover-preview-renderer.ts
...
function createPdfCoverPreviewRenderer() {
  ...
  const pdfPackageRoot = import_node_path2.default.dirname(runtimeRequire.resolve("pdfjs-dist/package.json"));
  const pdfModuleUrl = (0, import_node_url.pathToFileURL)(
    import_node_path2.default.join(pdfPackageRoot, "legacy", "build", "pdf.mjs")
  ).href;
  const workerModuleUrl = (0, import_node_url.pathToFileURL)(
    import_node_path2.default.join(pdfPackageRoot, "legacy", "build", "pdf.worker.mjs")
  ).href;
  const wasmUrl = `${(0, import_node_url.pathToFileURL)(import_node_path2.default.join(pdfPackageRoot, "wasm")).href}/`;
...
function createCoverPreviewWarmQueue(ctx) {
  ...
  const pump = () => {
    ...
    while (activeCount < COVER_PREVIEW_WARM_CONCURRENCY && queuedItemIds.length) {
      ...
      activeCount += 1;
      void ctx.lifecycle.run("books.cover-preview", () => generateCoverPreview(itemId)).catch((error) => {
...
var booksPlugin = {
  ...
  activate(ctx) {
    ...
    ctx.ipc.handle("get-cover-preview", async (_event, payload) => {
      try {
    ...
    ctx.ipc.handle("get-by-item-id", async (_event, payload) => {
      try {
        const itemId = typeof payload === "string" ? payload : String(payload?.itemId || "");
    ...
    ctx.ipc.handle("update", async (_event, payload) => {
      try {

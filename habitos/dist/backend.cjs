...
var OCCURRENCE_STATUS_VALUES = /* @__PURE__ */ new Set(["pending", "completed", "failed"]);
...
function buildHistoryItemFromOccurrence(occurrence, habit) {
  return {
    ...
    summary: occurrence.status === "completed" ? "Ocurrencia registrada como cumplida." : "La ventana del habito cerro sin completarse.",
    occurrence,
    habit,
    raw: occurrence
  };
...
function buildHabitosHomeSnapshot(sqlite, options = {}) {
  ...
  const dailyTaskItems = tasks.filter((task) => task.status === "open" && compareLocalDates(task.dueDate, today) <= 0).map((task) => buildTaskQueueItem(task, today));
  const dailyHabitItems = occurrences.filter((occurrence) => occurrence.status === "pending" && occurrence.occurrenceDate === today).map((occurrence) => {
    const habit = habitsById.get(occurrence.habitId);
    return habit ? buildOccurrenceQueueItem(occurrence, habit, today) : null;
  }).filter(Boolean);
  const upcomingTasks = tasks.filter((task) => task.status === "open" && compareLocalDates(task.dueDate, today) > 0).sort(compareUpcomingTasks).slice(0, DEFAULT_UPCOMING_LIMIT);
  const recentHistory = [
    ...tasks.filter((task) => task.status === "completed" || task.status === "failed").map(buildHistoryItemFromTask),
    ...occurrences.filter((occurrence) => occurrence.status === "completed" || occurrence.status === "failed").map((occurrence) => buildHistoryItemFromOccurrence(occurrence, habitsById.get(occurrence.habitId)))
  ].sort(compareHistoryEntries).slice(0, DEFAULT_HISTORY_LIMIT);
  const completedTaskIdsToday = new Set(
    tasks.filter((task) => task.status === "completed" && String(task.completedAt || "").startsWith(today)).map((task) => task.id)
  );
